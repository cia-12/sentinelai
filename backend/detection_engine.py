...
# ─── Isolation Forest (lightweight, no sklearn dependency) ──────────────────
    ...
    if threat_type == "brute_force":
        features = [
            {"feature": "login_rate_per_min", "value": rate,
             ...
             "explanation": f"{rate} auth attempts in 60s (threshold: {BRUTE_FORCE_THRESHOLD})"},
        ...
        features = [
            ...
            {"feature": "dst_ip_threat_intel",
             ...
             "explanation": f"Destination matches Emerging Threats C2 feed"},
...
class DetectionEngine:
    def __init__(self):
        ...
        self.seq = SequenceAnalyzer(window=15, regularity_threshold=C2_BEACON_THRESHOLD)
    ...
    def process(self, event: dict) -> Optional[ThreatAlert]:
        ...
        if layer == "http" and event.get("endpoint") == "/api/login":
            ...
            if count > max(15, BRUTE_FORCE_THRESHOLD // 3) or global_count > BRUTE_FORCE_THRESHOLD:
                ...
                alert = ThreatAlert(
                    ...
                    why_flagged=f"Authentication rate of {global_count} requests/min exceeds threshold of {BRUTE_FORCE_THRESHOLD}. "
        ...
        if layer == "network" and dst_ip in THREAT_INTEL_IPS:
            ...
            if bscore > C2_BEACON_THRESHOLD and binfo.get("beacon_count", 0) >= 4:

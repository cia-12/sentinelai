...
class C2BeaconAttack:
    ...
    def __init__(self):
        ...
        self.interval_s = 8  # increased from 5 for manageability (real = 300s)
        self.last_beacon = 0
        self.beacon_count = 0

    def should_fire(self) -> bool:
        now = time.time()
        jitter = random.uniform(-0.3, 0.3)  # increased jitter range
...
# ─── Main generator (mixes benign + attack traffic) ──────────────────────────
...
class LogGenerator:
    ...
    def _pop_manual_batch(self):
        scenario = self._manual_scenario
        self._manual_scenario = None
        if scenario == "brute_force":
            return [self.brute.next_event()]
        if scenario == "c2_beacon":
            return [self.c2.next_event()]
        if scenario == "lateral_movement":
            return self.lateral.next_event()
        if scenario == "data_exfil":
            return self.exfil.next_event()
        if scenario == "false_positive":
            return [self.fp.next_event()]
        return []
    ...
    def _build_schedule(self):
        ...
        now = time.time()
        return {
            "brute":   now + 5,      # starts after 5s
            "c2":      now + 0,      # immediate (interval-controlled at 5s)
            "lateral": now + 15,     # increased from 8s
            "exfil":   now + 25,     # increased from 15s
            "fp":      now + 30,     # increased from 20s
        }
    ...
    def generate(self) -> Generator[list, None, None]:
        ...
        while True:
            batch = []
            if self._manual_scenario:
                batch.extend(self._pop_manual_batch())
            now = time.time()
            ...
            # Attack injections based on schedule (throttled for clarity)
            if now >= self._attack_schedule["brute"]:
                batch.append(self.brute.next_event())
                self._attack_schedule["brute"] = now + random.uniform(0.1, 0.3)  # slower rate

            if self.c2.should_fire():
                batch.append(self.c2.next_event())

            if now >= self._attack_schedule["lateral"] and random.random() < 0.2:  # reduced 0.3->0.2
                batch.extend(self.lateral.next_event())
                self._attack_schedule["lateral"] = now + random.uniform(2, 4)  # slower

            if now >= self._attack_schedule["exfil"] and random.random() < 0.15:  # reduced 0.2->0.15
                batch.extend(self.exfil.next_event())
                self._attack_schedule["exfil"] = now + random.uniform(3, 7)  # slower

            if now >= self._attack_schedule["fp"] and random.random() < 0.1:  # reduced 0.15->0.1
                batch.append(self.fp.next_event())
                self._attack_schedule["fp"] = now + random.uniform(4, 8)  # slower
